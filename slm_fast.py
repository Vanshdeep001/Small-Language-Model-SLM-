#!/usr/bin/env python3
"""
Fast Small Language Model (SLM) Training Script
Optimized for CPU training with reduced parameters for quick results
"""
...
if not os.path.exists("train.bin"):
    ...
    tokenized = ds.map(
        ...
        num_proc=2,  # Reduced for CPU stability
    )
...
max_iters = 1000      # Much fewer iterations
warmup_steps = 100    # Shorter warmup
min_lr = 1e-4
eval_iters = 50       # Less frequent evaluation
batch_size = 8        # Smaller batch for CPU
block_size = 32       # Smaller context window
...
# Smaller model configuration for speed
config = GPTConfig(
    vocab_size=50257,
    block_size=32,    # Smaller context
    n_layer=2,        # Only 2 layers
    n_head=2,         # Only 2 heads
...
plt.ylabel("Loss")
plt.title("Fast SLM Training and Validation Loss")
plt.legend()
plt.grid(True)
plt.savefig('fast_training_losses.png')
plt.show()
...
print("\n" + "="*60)
print("FAST SLM INFERENCE RESULTS")
print("="*60)
...
for i, sentence in enumerate(test_prompts, 1):
    ...
    with torch.no_grad():
        y = model.generate(context, 30, temperature=0.8, top_k=50)
...
print(f"Training plot saved as: fast_training_losses.png")
print("="*60)
