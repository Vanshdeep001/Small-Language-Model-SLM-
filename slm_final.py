#!/usr/bin/env python3
"""
Final Working Small Language Model (SLM) Training Script
Windows-compatible with checkpoint resuming
"""
...
if __name__ == '__main__':
    ...
    if not os.path.exists("train.bin"):
        ...
        # Process function with tokenizer in scope
    ...
    # Training parameters
    learning_rate = 1e-4
    max_iters = 2000  # Reduced for faster completion
    warmup_steps = 200
    min_lr = 5e-4
    eval_iters = 50
    batch_size = 16
    ...
    def load_checkpoint(model, optimizer, scheduler):
        ...
        if os.path.exists('training_checkpoint.pt'):
            checkpoint = torch.load('training_checkpoint.pt', map_location=device)
    ...
    start_epoch, best_val_loss, train_loss_list, validation_loss_list = load_checkpoint(model, optimizer, scheduler)

    # Training loop
    print("Starting training...")
    print(f"Expected time: 1-2 hours")
    print(f"Training for {max_iters} iterations with evaluation every {eval_iters} steps")
    print(f"Starting from epoch {start_epoch}")

    best_model_params_path = "best_model_params.pt"

    for epoch in tqdm(range(start_epoch, max_iters), desc="Training", initial=start_epoch, total=max_iters):
        if epoch % eval_iters == 0 and epoch != 0:
            ...
            if epoch % 100 == 0:
                save_checkpoint(model, optimizer, scheduler, epoch, best_val_loss, train_loss_list, validation_loss_list)
    ...
    plt.ylabel("Loss")
    plt.title("Final SLM Training and Validation Loss")
    plt.legend()
    plt.grid(True)
    ...
    print("\n" + "="*60)
    print("FINAL SLM INFERENCE RESULTS")
    print("="*60)
    ...
    print(f"Training plot saved as: final_training_losses.png")
    print("Checkpoint saved as: training_checkpoint.pt")
    print("="*60)
